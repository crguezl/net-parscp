...
SKIP: {
  skip("Developer test", $totaltests-1) unless ($ENV{DEVELOPER} && -x "script/parpush" && ($^O =~ /nux$/));
     ...
     $output = `script/parpush -l orion=ORI -l beowulf=BEOW -v 'orion:.bashrc beowulf:.bashrc' :/tmp/bashrc.@# 2>&1`;
     ok(!$?, 'macro for source with 2 -l options (to local): status 0');
     like($output, qr{scp -r beowulf:.bashrc /tmp/bashrc.BEOW}, 'macro for source with 2 -l options (to local): correct command 1');
     like($output, qr{scp -r orion:.bashrc /tmp/bashrc.ORI}, 'macro for source with 2 -l options (to local): correct command 2');
